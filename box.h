 ...
 //     assert(min < max);
	...
	// implement for Homework Project
	 bool overlap(const Box &box) {
		 ...
		 if (parameters[0].x() < box.parameters[1].x() && box.parameters[0].x() < parameters[1].x()) {
			 if (parameters[0].y() < box.parameters[1].y() && box.parameters[0].y() < parameters[1].y()) {
				 if (parameters[0].z() < box.parameters[1].z() && box.parameters[0].z() < parameters[1].z()) {
					 return true;
				 }
			 }
		 }
