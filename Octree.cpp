...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	float timestart = ofGetElapsedTimeMillis();
	level++;
    subdivide(mesh, root, numLevels, level);
	float timeEnd = ofGetElapsedTimeMillis();
	cout << "Time to Build Octree: " << timeEnd - timestart << " milliseconds" << endl;
}
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < boxes.size(); i++)
	{
		...
		int pointCount = getMeshPointsInBox(mesh,node.points, boxes[i], childNode.points);
		//if a child box contains at least 1 point add child to tree
		if (pointCount > 0) {
			childNode.box = boxes[i];
			node.children.push_back(childNode);
			//if child is not a leaf node (contains more than 1 point) recursively call subdivide(child)
			if (pointCount > 1) {
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	if (node.box.intersect(ray, -1000, 1000)) {
		...
		float timeEnd = ofGetElapsedTimeMillis();
	//	cout << "Time for searching intersect data  " << timeEnd - timestart << " milliseconds" << endl;
		return intersects;
	}
